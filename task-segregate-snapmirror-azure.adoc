...
= Segregate SnapMirror traffic in Cloud Volumes ONTAP in Azure
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
.. Save the privateIPAddress. This IP address, <new_added_nic_primary_addr>, is used to create an intercluster LIF in <<step-second,broadcast domain, and intercluster LIF for the new NIC>>.
...
== [[step-second]]Step 2: Create a new IPspace, broadcast domain, and intercluster LIF for the new NIC
