...
[.lead]
You can deploy Cloud Volumes ONTAP in Amazon Web Services (AWS) and Microsoft Azure using quick creat that faster deployment in both single node and high availability (HA) configurations. This simplified process reduces deployment steps compared to the advanced method by automatically setting default values on a single page and minimizing navigation.

.Before you begin

You need the following to add a Cloud Volumes ONTAP system from the NetApp Console.

[[licensing]]


. A Console agent that's up and running. You should have a Console agent that is associated with your project or workspace. You should be prepared to leave the Console agent running at all times. For information, refer to these links:
...
. An understanding of the configuration that you want to use. You should have prepared by choosing a configuration and by obtaining networking information from your administrator. For information, refer to these links:
+
[role="tabbed-block"]
====
.AWS
...
. An understanding of what's required to set up licensing for Cloud Volumes ONTAP. For information, refer to these links:
+
[role="tabbed-block"]
====
.AWS
...
. DNS and Active Directory for CIFS configurations. For information, refer to these links:
+
[role="tabbed-block"]
====
.AWS
...
. Select the provider > *Cloud Volumes ONTAP* > *Add new*. The *Quick create* option is selected by default.
...
. *Name*: The system name. The Console uses the system (cluster) name to name the Cloud Volumes ONTAP system and the VM instance. It also uses the name as the prefix for the predefined security group, if you select that option.
...
. *Tags* Tags are the metadata for your AWS or Azure resources. The Console adds the tags to the Cloud Volumes ONTAP instance and each resource associated with the instance. You can add up to 15 tags from the user interface when creating a Cloud Volumes ONTAP system, and then you can add more after its created. Note that the API does not limit you to four tags when creating a system.
For information about tags, refer to 
...
.. *Region*: By default, the region of the associated cloud account that has VPC with subnet resources is selected. 
.. *Availability zone*: ##If you select single node deployment, you can select an availability zone. If you select HA in a single AZ, you must select an availability zone. If you select HA in multiple AZs, the availability zones for the nodes are automatically selected based on the VPC and subnet selection.##
.. *VPC*: Enter a VPC for the region with a subnet. If there are no subnets, then the default value for the VPC is selected.
...
. *Marketplace Subscription*: Select the AWS or Azure marketplace subscription you want to use with this Cloud Volumes ONTAP system.
