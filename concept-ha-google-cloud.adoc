...
* Single availability zone (AZ) or multiple availability/three zones (recommended).
+
...
=== Mediator
##Traditionally, Google Cloud HA configurations in Cloud Volumes ONTAP use a non shared model, where a mediator is used to manage failover operations or monitor node health. This requires creation of an additional VM in Google Cloud and two sets of disks for each node.## Here are some key details about the mediator instance in Google Cloud:
...
Upgrades:: When you upgrade Cloud Volumes ONTAP, the NetApp Console also updates the mediator instance as needed.
...
##Cloud Volumes ONTAP 9.19.1 and later support shared for single AZ HA configurations. With Shared HA, a disk is shared between both the nodes. So, the method of aggregate creation changes. Instead of creating two disks for two VMs, a single disk is created and attached to the VMs. Unlike non-Shared HA, shared HA doesn't need the disk mirror.##
...
Multiple availability zones (recommended)::
...
When deployed in a single availability zone (AZ), a Cloud Volumes ONTAP HA configuration uses a spread placement policy. This policy ensures that an HA configuration is protected from a single point of failure within the zone, without having to use separate zones to achieve fault isolation.
+
