...
=== Mediator
##Traditionally, Cloud Volumes ONTAP HA configurations in Google Cloud use a non-shared model, where a mediator instance is used to manage failover operations or monitor node health. This needs creating an additional VM in Google Cloud and two sets of disks for each node.## 
...
##Cloud Volumes ONTAP 9.19.1 and later support shared HA configuration for single AZ. In the shared HA architecture, both the nodes can share a disk, whereby, the method of aggregate creation changes. Instead of creating two disks for two VMs, a single disk is created and attached to the VMs. Shared HA doesn't need the disk mirror.##
