...
. From the NetApp Console, use the API to create a Cloud Volumes ONTAP system that uses the key.
...
https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html[View the latest list of permissions^]

.Steps

. Obtain the list of key vaults in your Azure subscription by using the following API calls.
+
https://docs.netapp.com/us-en/console-automation/cvo/get-azure-ha-metadata-keys-vault.html[For an HA pair^]: `GET /azure/ha/metadata/vaults`
+
https://docs.netapp.com/us-en/console-automation/cvo/get-azure-vsa-metadata-keys-vault.html[For single node^]: `GET /azure/vsa/metadata/vaults`
+
Make note of the *name* and *resourceGroup*. You'll need to specify those values in the next step.
. Obtain the list of keys within the vault by using the following API call.
+
For an HA pair: `GET /azure/ha/metadata/keys-vault?resourceGroupName=KeyVaultResourceGroup&vaultName=KeyVault`
+
For single node: `GET /azure/vsa/metadata/keys-vault?resourceGroupName=KeyVaultResourceGroup&vaultName=KeyVault`
+
Make note of the *keyName*. You'll need to specify that value (along with the vault name) in the next step.
. Create a Cloud Volumes ONTAP system by using the following API call.

.. For an HA pair use https://docs.netapp.com/us-en/console-automation/cvo/post-azure-ha-working-environments.html[this API call^]:
+
...
+
.. For a single-node system use https://docs.netapp.com/us-en/console-automation/cvo/post-azure-vsa-working-environments.html[this API call^]:
+
...
* https://docs.netapp.com/us-en/console-automation/cm/wf_cvo_before.html[Prepare to use the Cloud Volumes ONTAP API workflows^].
* https://docs.netapp.com/us-en/console-automation/cm/api_ref_resources.html#azure-haworking-environments[Learn about the ONTAP management REST APIs^].