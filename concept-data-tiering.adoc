...
+
The NetApp Console creates a single S3 bucket for each system and names it fabric-pool-_cluster unique identifier_. A different S3 bucket is not created for each volume.
+
...
+
You can select a storage class when you create the system and you can change it any time afterwards. For instructions on changing the storage class, refer to link:task-tiering.html[Tier inactive data to low-cost object storage].
+
...
+
You can select a storage tier when you add a Cloud Volumes ONTAP system and you can change it any time afterwards. For details about changing the storage tier, refer to link:task-tiering.html[Tier inactive data to low-cost object storage].
+
...
When you enable data tiering in Google Cloud, Cloud Volumes ONTAP uses persistent disks as a performance tier for hot data and a Google Cloud Storage bucket as a capacity tier for inactive data. You can select a storage tier when you create a system and change it any time afterwards. For information about changing the storage class, refer to link:task-tiering.html[Tier inactive data to low-cost object storage].
...
+
The Console creates a bucket for each system and names it fabric-pool-_cluster unique identifier_. A different bucket is not created for each volume.
+
...
Storage classes:: Google Cloud uses https://cloud.google.com/storage/docs/storage-classes[storage classes^] to manage costs and data availability. We recommend that you select a storage class based on your expected data access frequency, because accessing data incurs higher costs. The default storage class for tiered data is _Standard Storage_. Unless manually changed, all inactive data maintains the default storage class. To change the storage class of inactive data, refer to https://docs.cloud.google.com/storage/docs/managing-lifecycles[Google Cloud documentation: Managing object lifecycles^].
+
The storage class for data tiering is system wide--it's not per volume.
endif::gcp[]
