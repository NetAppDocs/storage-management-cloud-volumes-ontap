...
Storage classes:: The default storage class for tiered data is _Standard Storage_. If a set of data is infrequently accessed, you can reduce your storage costs by changing its storage class to _Nearline Storage_ or _Coldline Storage_. Unless manually changed, all inactive data maintains the default storage class. To change the storage class of inactive data, refer to https://docs.cloud.google.com/storage/docs/managing-lifecycles[Google Cloud documentation: Managing object lifecycles^].     
+
The access costs are higher if you do access the data, so take that into consideration before you change the storage class. To learn more, refer to the https://cloud.google.com/storage/docs/storage-classes[Google Cloud documentation: Storage classes^].
+
You can select a storage tier when you create the system and you can change it any time afterwards. For details about changing the storage class, refer to link:task-tiering.html[Tier inactive data to low-cost object storage].
+
The storage class for data tiering is system wide--it's not per volume.
endif::gcp[]
