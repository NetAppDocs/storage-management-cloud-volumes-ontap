...
[CAUTION]
During discovery, you might see failure messages, but you can ignore them until the discovery process is complete. Do not modify the configuration of the Cloud Volumes ONTAP systems in the Azure marketplace portal during discovery, especially the system tags. Any changes made to these configurations may lead to unexpected system behavior.
