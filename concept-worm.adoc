...
The WORM feature is available for use with your marketplace subscriptions and Bring-Your-Own License (BYOL) at no additional cost. Contact your NetApp sales representative to add WORM to your current license.
...
You can use an application to commit files to WORM over NFS or CIFS, or use the ONTAP CLI to autocommit files to WORM automatically. You can also use a WORM appendable file to retain data that is written incrementally, like log information. For information, refer to https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually[Commit files to WORM using ONTAP SnapLock^].

After you activate WORM storage on a Cloud Volumes ONTAP system, you must use the ONTAP CLI for all management of WORM storage. For instructions, refer to the link:https://docs.netapp.com/us-en/ontap/snaplock/index.html[ONTAP documentation on SnapLock^].
...
* link:concept-licensing-charging.html[Licensing and charging in Cloud Volumes ONTAP]
