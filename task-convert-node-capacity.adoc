...
Because marketplace subscriptions are not supported in private mode, you need to purchase an Essentials (bring your own license (BYOL)) package from NetApp. 
[NOTE]
Note that the conversion from node-based licenses to any package other than Essentials is not supported for private mode.

You can only convert your existing node-based BYOL licenses to capacity-based Essentials BYOL packages using the license conversion tool in the NetApp Console. This lets you keep the benefits of capacity-based licensing within the BYOL model without requiring marketplace subscriptions.

.About this task

* After conversion, the entire capacity of your node-based license is tracked under the Essentials package, and you continue to be billed through your BYOL agreement. Conversion to other capacity-based licenses, such as Professional packages, is not supported in private mode.
...
** The account and application data in your Cloud Volumes ONTAP system remains intact.
** The underlying cloud resources remain unaffected post conversion.
* The license conversion tool supports all deployment types, such as single node, high availability (HA) in single availability zone (AZ), and HA in multiple AZ.
