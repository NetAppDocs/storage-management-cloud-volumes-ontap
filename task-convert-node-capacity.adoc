...
You can also convert your existing BYOL node-based licenses to an Essentials BYOL package of the same capacity by using the license conversion tool in the NetApp Console. This lets you keep the benefits of capacity-based licensing within the BYOL model without requiring marketplace subscriptions.

.About this task

* You can only convert node-based licenses to capacity-based Essentials packages using BYOL licensing through the license conversion tool in the NetApp Console. 
* This approach allows you to retain the benefits of capacity-based licensing within the BYOL model.
...
* The license conversion tool supports all deployment types, such as single node, high availability (HA) in single availability zone (AZ), HA in multiple AZ, bring your own license (BYOL), and PAYGO.
* The conversion is supported for all cloud providers, AWS, Azure, and Google Cloud.
* Post conversion, the serial number of the node-based license will be replaced by a capacity-based format. This is done as a part of the conversion. 
* When you transition to the capacity-based model, your data continues to be retained in the same location as the node-based licensing. This approach guarantees no disruption in data placement, and upholds data sovereignty principles throughout the transition.

.Before you begin

* Private mode Cloud Volumes ONTAP deployments don't support marketplace subscriptions. 
* You must have procured a capacity-based Essentials package with at least 500 GiB capacity from NetApp to convert your node-based license to a capacity-based license.
* The capacity-based Essentials package should correspond to your node-based license type. For example, to convert a BYOL Secondary HA node-based license, you should already have an Essentials Secondary HA package available in your account. 
...
. If you have a BYOL license, you can select the checkbox to delete the node-based license from the Console after the conversion is complete. If the conversion is still in progress, selecting this checkbox will not remove the license from the Console.
. Select the check box to confirm that you understand the implications of the change, and then click *Proceed*.
