...
summary: After the end of availability (EOA) of your node-based licenses, you should transition to capacity-based licensing by using the license conversion tool.
---
...
If you have a standard mode deployment, you can convert your node-based license to a capacity-based license by using the license conversion tool in the NetApp Console. The conversion is supported for all cloud providers, AWS, Azure, and Google Cloud. You can convert your node-based license to any capacity-based license purchased through the marketplace. For example, if you have a PAYGO Standard node-based license, you can convert it to any capacity-based license purchased through the marketplace.
...
* It is a non-disruptive operation:
** No service disruption or application downtime occurs.
...
* For standard mode, the tool supports all node-based licenses as the source and all capacity-based licenses as the destination. For example, if you have a BYOL node-based license, you can convert it to any capacity-based license purchased from NetApp.
* Post conversion, the serial number of the node-based license is replaced by a capacity-based format. This is done as a part of the conversion, and is reflected on your NetApp Support Site (NSS) account. 
...
. On the *Systems* page, double-click the name of the system for which you want to modify the license type.
...
+
. On the *Convert Node-based licenses to Capacity-based* screen, verify the system name and source license details.
. Select the destination package for converting the existing license:
** Essentials. The default value is `Essentials`.
** Professional
. If you have a BYOL license, you can select the checkbox to delete the node-based license from the Console after the conversion is complete. If the conversion is still in progress, selecting this checkbox will not remove the license from the Console. This option is not available for marketplace subscriptions.
. Select the check box to confirm that you understand the implications of the change, and then click *Proceed*.

.After you finish
View the new license serial number and verify the changes in the *Administration > Licenses and subscriptions* menu of the Console.


== Conversion in a private-mode deployment

If you have a private-mode deployment, you can convert your node-based license to a capacity-based license by using the license conversion tool in the NetApp Console. The conversion is supported for all cloud providers, AWS, Azure, and Google Cloud. 

Because marketplace subscriptions are not supported in private mode, you need to purchase an Essentials Bring-Your-Own License or BYOL package from NetApp. This lets you keep the benefits of capacity-based licensing within the BYOL model without requiring marketplace subscriptions.
[NOTE]
The conversion from node-based licenses to any capacity-based license other than Essentials is not supported for private mode.



.About this task

* After conversion, the entire capacity of your node-based license is tracked under the Essentials package, and you continue to be billed through your BYOL agreement. 
* When you use the license conversion tool, the transition from node-based to capacity-based licensing model is carried out in place that eliminates the need for any data migration or provisioning of additional cloud resources.
* It is a non-disruptive operation: 
** No service disruption or application downtime occurs.
** The account and application data in your Cloud Volumes ONTAP system remains intact.
** The underlying cloud resources remain unaffected post conversion.
** Your data continues to be retained in the same location as the node-based licensing. This approach guarantees no disruption in data placement, and upholds data sovereignty principles throughout the transition.
* The license conversion tool supports all deployment types, such as single node and high availability (HA) configuration in single availability zone (AZ) and multiple AZ.
* Post conversion, the Cloud Volumes ONTAP systems are updated with new serial numbers.


.Before you begin

* Because private mode Cloud Volumes ONTAP deployments don't support marketplace subscriptions, you must procure your capacity-based Essentials license from NetApp. 
* The capacity-based license should have a similar capacity as the node-based license for the conversion.
* The capacity-based Essentials package should correspond to your node-based BYOL license type. For example, a private-mode deployment with secondary volume (HA) should have the respective Essentials Secondary HA license or a license with the best match, such as Essentials Primary, Essentials Primary HA.
...
. On the *Systems* page, double-click the name of the system for which you want to modify the license type.
. On the *Overview* tab, click the *Features* panel.
. Check the pencil icon next to *Charging method*. If the charging method for your system is `Node Based`, you can convert it to a capacity-based license. 
+
[TIP]
The icon is disabled if your Cloud Volumes ONTAP system is already charged by capacity, or if any of the requirements is not fulfilled. 
+
. On the *Convert Node-based licenses to Capacity-based* screen, verify the system name and source license details.
. Select Essentials as the destination package for converting the existing license:
. You can select the checkbox to delete the node-based license from the Console after the conversion is complete. If the conversion is still in progress, selecting this checkbox will not remove the license from the Console.
. Select the check box to confirm that you understand the implications of the change, and then click *Proceed*.
...
For details on pricing, go to the https://bluexp.netapp.com/pricing/[NetApp Console website^].
...
* link:concept-licensing.html[Licensing for Cloud Volumes ONTAP]
* link:task-manage-capacity-licenses.html[Manage capacity-based licensing for Cloud Volumes ONTAP]
* link:task-manage-node-licenses.html[Manage node-based licensing]
