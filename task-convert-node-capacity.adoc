...
* *Private mode considerations*: If you have private mode Cloud Volumes ONTAP deployments, you can convert your existing BYOL node-based licenses to an Essentials package using the license conversion tool in the NetApp Console. This transition lets you retain the benefits of capacity-based licensing within the BYOL model without requiring marketplace subscriptions. You can convert a node-based license to the corresponding Essentials package, where the capacity remaining in the node-based license continues to be charged through the converted Essentials package. For example, if you have a BYOL Secondary HA node-based license, you can run the conversion only if you have a Essentials Secondary HA package. The conversion from node-based licenses to any other capacity-based license, such as a Professional package, is not supported for private mode.## 
