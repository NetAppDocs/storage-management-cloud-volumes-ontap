...
image:diagram_ha_azure.png["A conceptual image that shows a load balancer managing incoming traffic from the NetApp Console and clients, two Cloud Volumes ONTAP nodes in an VMSS Flex, SSD disks for boot data, and Page Blobs for customer data."]
...
Beginning with Cloud Volumes ONTAP 9.15.1, you can create and manage heterogeneous virtual machines (VMs) in a single availability zone (AZ). You can deploy high-availability (HA) nodes in separate fault domains within the same AZ, guaranteeing optimal availability. To learn more about the flexible orchestration mode that enables this capability, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Microsoft Azure documentation: Virtual Machine Scale Sets^].

Disks::
Customer data resides on Premium Storage page blobs. Each node has access to the other node's storage. Additional storage is also required for link:reference-configs-azure.html#azure-ha-pair[boot, root, and core data].
...
image:diagram_ha_azure_saz_lrs.png["A conceptual image that shows a load balancer managing incoming traffic from the Console and clients, two Cloud Volumes ONTAP nodes in  VMSS Flex, SSD disks for boot data, and LRS managed disks for customer data."]
...
Disks::
Customer data resides on locally-redundant storage (LRS) managed disks. Each node has access to the other node's storage. Additional storage is also required for link:reference-configs-azure.html#azure-ha-pair[boot, root, partner root, core, and NVRAM data].
...
image:diagram_ha_azure_maz.png["A conceptual image that shows a load balancer managing incoming traffic from the Console and clients, two Cloud Volumes ONTAP nodes in two availability zones, SSD disks for boot data, and managed disks for customer data."]
...
Availability Zones::
HA multiple availability zone configuration utilizes a deployment model where two Cloud Volumes ONTAP nodes are deployed into different availability zones, ensuring that the nodes are in different fault domains to provide redundancy and availability. To learn how Virtual Machine Scale Sets in Flexible orchestration mode can use availability zones in Azure, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-use-availability-zones?tabs=cli-1%2Cportal-2[Microsoft Azure documentation: Create a Virtual Machine Scale Set that uses Availability Zones^].

Disks::
Customer data resides on zone-redundant storage (ZRS) managed disks. Each node has access to the other node's storage. Additional storage is also required for link:reference-configs-azure.html#azure-ha-pair[boot, root, partner root, and core data].
...
[NOTE]
Cloud Volumes ONTAP uses load balancers in active-passive mode for probes. In Azure, only the active node responds to a probe. The passive node may show up as inactive with an X mark. This is expected behavior and not a cause for concern.
