...
The Cloud Volumes ONTAP version you can upgrade to depends on the version you are currently running. Unless a specific patch or release is listed, each generic version in the following tables represents the highest patch level available for that release that you can upgrade to.
