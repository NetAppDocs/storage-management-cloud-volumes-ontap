...
The version of Cloud Volumes ONTAP that you can upgrade to depends on the version of Cloud Volumes ONTAP that you're currently running. Unless any specific patch or release version is mentioned, the generic versions mentioned in the following tables indicate the highest patch available for that version to upgrade. 
