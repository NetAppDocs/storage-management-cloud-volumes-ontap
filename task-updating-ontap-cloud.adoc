...
For patch versions 9.11.1 or below, you will need to use a manual upgrade procedure by <<Upgrade from an image available at a URL,downloading the ONTAP image>>.
...
link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-change-ec2-instance.html[Change the EC2 instance type for Cloud Volumes ONTAP^]
...
. On the required aggregate tile, click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon, and then select *View Aggregate details*.
+
...
. On the system, click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon, and then click *Update Cloud Volumes ONTAP*.
