...
summary: The NetApp Console handles the set up of networking components for Cloud Volumes ONTAP, such as IP addresses, netmasks, and routes. You need to make sure that outbound internet access is available, that enough private IP addresses are available, that the right connections are in place, and more.
---
...
| 
The exact commercial endpoint for AWS service (suffixed with `amazonaws.com`) depends on the AWS region that you are using. Refer to the https://docs.aws.amazon.com/general/latest/gr/rande.html[AWS documentation for details^].
...
During Cloud Volumes ONTAP deployment, the AWS mediator proxy field accepts a server address. If you enter a fully qualified domain name (FQDN) instead of an IP address, the FQDN will fail to resolve unless the AWS VPC is configured with a custom DNS server. This is because the default AWS-provided DNS only resolves public DNS records and internal AWS hosts and can't resolve external FQDNs such as customer proxy servers. If this scenario applies to you, you have two options:

* Add the proxy server's IP address directly in the mediator proxy field instead of the FQDN.
* Configure a DHCP options set in your VPC that includes your custom DNS server addresses. This allows AWS to resolve the FQDN when you enter it in the proxy server field during deployment. Refer to the https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html[AWS documentation^] for details.
...
You don't need to create security groups because the Console does that for you. If you need to use your own, refer to link:reference-security-groups.html[Security group rules].

TIP: Looking for information about the Console agent? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html[View security group rules for the Console agent^]
...
+
NOTE: The Console automatically creates static IP addresses for iSCSI access and for NAS access from clients outside the VPC. You don't need to meet any requirements for these types of IP addresses.
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html[View networking requirements for the Console agent^]
