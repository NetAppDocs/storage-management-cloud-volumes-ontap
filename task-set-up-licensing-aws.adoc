...
If you have a standalone AWS subscription linked to your AWS account, you can pay for Cloud Volumes ONTAP through that subscription. If you also have NetApp Intelligent Services (NIS) subscriptions, you can pay for your data services through the package associated with the NIS subscription. For existing subscriptions, NetApp recommends switching to the standalone subscription approach for better cost management of your Cloud Volumes ONTAP usage.

.About this task

If you are an existing customer with NIS subscriptions associated with the AWS account, NetApp recommends switching to the standalone subscription approach for better cost management of your Cloud Volumes ONTAP usage. This is the complete process:
...
. (If applicable) Convert your Professional package to an Essentials package and set the billing preference. Refer to https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-capacity-licenses.html#change-charging-methods[Change charging methods for capacity licenses^] for information about converting your Professional package to an Essentials package.
. Continue billing data services through your NIS subscription.


.Steps
. On the left panel of the NetApp Console, select *Administration > Licenses and subscriptions > Billing preferences > Cloud Volumes ONTAP*.
. On the AWS tab, select the standalone subscription and click the options icon image:screenshot_gallery_options.gif[kabob icon].
. On the Edit configuration window, select the subscription number of the standalone listing in the *AWS account credentials* field.
. Click *Configure*. You can see the subscription as the preferred option for your Cloud Volumes ONTAP billing in AWS. 
. When deploying a new Cloud Volumes ONTAP system, select the Essentials package to bill through the standalone subscription. For information, refer to https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-aws.html[Deploy Cloud Volumes ONTAP with Quick create in AWS^].

Repeat the same steps for the NIS subscription on the *Data services* tab to use the NIS subscription for your data services. When deploying a new Cloud Volumes ONTAP system, select the Professional package to bill through the NIS subscription.
...
* A license purchased from NetApp (Bring-Your-Own License or BYOL)
...
. https://docs.netapp.com/us-en/console-setup-admin/task-adding-nss-accounts.html#add-an-nss-account[Add your NetApp Support Site account to the Console^]
+
...
+
Your license must be available from the Console before you can use it with Cloud Volumes ONTAP. If needed, you can link:https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-data-services-licenses.html#add-a-license[manually add the license to the Console^].
...
TIP: You can manage the AWS Marketplace subscriptions associated with your AWS accounts from the Settings > Credentials page. https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html[Learn how to manage your AWS accounts and subscriptions^]
...
Select the Freemium offering to use Cloud Volumes ONTAP free of charge with up to 500 GiB of provisioned capacity. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#free-trials[Learn more about the Freemium offering^].
...
.. On the *Details and Credentials* page, click *Edit Credentials > Add Subscription* and then follow the prompts to subscribe to the pay-as-you-go offering in the AWS Marketplace.
+
You won't be charged through the marketplace subscription unless you exceed 500 GiB of provisioned capacity, at which time the system is automatically converted to the link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages[Essentials package^].
+
image:screenshot-aws-paygo-subscription.png[A screenshot of the PAYGO subscription option when creating a Cloud Volumes ONTAP system.]
