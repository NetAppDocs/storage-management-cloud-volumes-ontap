...
= Deploy Cloud Volumes ONTAP using advanced options in AWS
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
** You should have a https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html[Console agent that is associated with your system^].
...
. Select *Amazon Web Services* and *Cloud Volumes ONTAP Single Node*.

. Select *Advanced create*. Because the *Quick create* mode is selected by default, you might see a message for default values. Click *Continue*.

. If you're prompted, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html[create a Console agent^].

. *Details and Credentials*: Optionally change the AWS credentials and subscription, enter a system name, add tags if needed, and then enter a password.
+
...
| Add tags | AWS tags are metadata for your AWS resources. The Console adds the tags to the Cloud Volumes ONTAP instance and each AWS resource associated with the instance.
...
https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html[Learn how to add additional AWS credentials to NetApp Console^].
...
image:screenshot_aws_marketplace.gif["A screenshot that shows the subscription page for the Console for Cloud Volumes ONTAP when the AWS account already has a subscription, but a particular IAM user might not."]
...
. *Location & Connectivity*: Enter the network information that you recorded in the https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#collect-networking-information[AWS worksheet^].
+
...
* If you choose *Selected VPC only*, the source for inbound traffic is the subnet range of the selected VPC and the subnet range of the VPC where the Console agent resides. This is the recommended option.
...
. *IAM Role*: It's best to keep the default option to let the Console create the role for you.
+
...
+
NOTE: If a newer Release Candidate, General Availability, or patch release is available for the selected version, then the Console updates the system to that version when creating the system. For example, the update occurs if you select Cloud Volumes ONTAP 9.13.1 and 9.13.1 P4 is available. The update does not occur from one release to another—for example, from 9.13 to 9.14.
...
. Select *Amazon Web Services* and *Cloud Volumes ONTAP HA*.
+
...
+
* https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html[Learn more about NetApp Data Classification^]
* https://docs.netapp.com/us-en/data-services-backup-recovery/task-backup-to-s3.html[Learn more about Backup and Recovery^]
+
TIP: If you would like to utilize WORM and data tiering, you must disable Backup and Recovery and deploy a Cloud Volumes ONTAP system with version 9.8 or above. 
...
. *Location and Connectivity* (single availability zone (AZ)) or *Region & VPC* (multiple AZs): Enter the network information that you recorded in the AWS worksheet.
+
The following table describes fields for which you might need guidance:
+
[cols=2*,options="header",cols="25,75"]
...
. *Usage Profile, Disk Type, and Tiering Policy*: Choose whether you want to enable storage efficiency features and edit the volume tiering policy, if needed.
+
For more information, refer to link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#choose-a-volume-usage-profile[Choose a volume usage profile^] and link:concept-data-tiering.html[Data tiering overview^].
...
* link:task-onboard-cvo-aws-marketplace.html[Subscribe and deploy Cloud Volumes ONTAP from AWS Marketplace]
* link:task-planning-your-config.html[Planning your Cloud Volumes ONTAP configuration]
* link:task-quick-deploy-aws.html[Deploy Cloud Volumes ONTAP in AWS using quick deployment]
