...
+
NOTE: Data tiering to Amazon Simple Storage Service (Amazon S3) is not supported if your Cloud Volumes ONTAP system is in an AWS Local Zone, because accessing the Amazon S3 buckets outside of the Local Zone involves higher latency and impacts Cloud Volumes ONTAP activities.
...
link:reference-configs-aws.html#aws[View the default disks for Cloud Volumes ONTAP system data in AWS].
