...
summary: You can launch a single Cloud Volumes ONTAP system in Azure by creating a Cloud Volumes ONTAP system in NetApp Console.
---

= Deploy Cloud Volumes ONTAP using advanced options in Azure
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
** You should have a https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html[Console agent that is associated with your system^].
...
+
You should have a configuration planned, and the necessary Azure networking details from your administrator. For more information, refer to link:task-planning-your-config-azure.html[Planning your Cloud Volumes ONTAP configuration^].
...
. If you're prompted, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html[create a Console agent^].
...
| Resource Group Tags | Tags are metadata for your Azure resources. When you enter tags in this field, the Console adds them to the resource group associated with the Cloud Volumes ONTAP system.
...
For information about tags, refer to the https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/tag-resources[Microsoft Azure Documentation: Using tags to organize your Azure resources^].
...
. *Location*: Select a region, availability zone, VNet, and subnet, and then select the checkbox to confirm network connectivity between the Console agent and the target location.
+
[NOTE]
For China regions, single node deployments are supported only in Cloud Volumes ONTAP 9.12.1 GA and 9.13.0 GA. You can upgrade these versions to later patches and releases of Cloud Volumes ONTAP as link:task-updating-ontap-cloud.html[supported in Azure]. If you want to deploy later Cloud Volumes ONTAP versions in China regions, contact NetApp Support. Only licenses purchased directly from NetApp are supported in China regions, marketplace subscriptions are not available.
+
...
TIP: If the Azure account that you're using has the https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html[required permissions^], the Console removes Cloud Volumes ONTAP resources from a resource group, in case of deployment failure or deletion.

| Generated security group a| If you let the Console generate the security group for you, you need to choose how you'll allow traffic:
...
| Use existing | If you choose an existing security group, then it must meet Cloud Volumes ONTAP requirements. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules[View the default security group^].
...
+
NOTE: If a newer Release Candidate, General Availability, or patch release is available for the selected version, then Console updates the system to that version when creating the working environment. For example, the update occurs if you select Cloud Volumes ONTAP 9.16.1 P3 and 9.16.1 P4 is available. The update does not occur from one release to another—for example, from 9.15 to 9.16.

. *Subscribe from the Azure Marketplace*: You see this page if the Console could not enable programmatic deployments of Cloud Volumes ONTAP. Follow the steps listed on the screen. refer to https://learn.microsoft.com/en-us/marketplace/programmatic-deploy-of-marketplace-products[Programmatic deployment of Marketplace products^] for more information.
...
+
For help choosing a disk type and size, refer to link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure[Sizing your system in Azure^].
...
+
This option is only available for certain VM types. To find out which VM types are supported, refer to link:link:reference-configs-azure.html#ha-pairs[Supported configurations by license for HA pairs]. 
+
...
+
For more information, refer to link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile[Understanding volume usage profiles^] and link:concept-data-tiering.html[Data tiering overview^].
...
+
* https://docs.netapp.com/us-en/bluexp-classification/concept-cloud-compliance.html[Learn more about NetApp Data Classification^]
* https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html[Learn more about NetApp Backup and Recovery ^]
+
TIP: If you would like to utilize WORM and data tiering, you must disable Backup and Recovery and deploy a Cloud Volumes ONTAP system with version 9.8 or above. 
...
You must use a dedicated resource group for each Cloud Volumes ONTAP HA pair that you deploy in Azure. Only one HA pair is supported in a resource group. The Console experiences connection issues if you try to deploy a second Cloud Volumes ONTAP HA pair in an Azure resource group.
...
* The disk size is for all disks in the initial aggregate and for any additional aggregates that the Console creates when you use the simple provisioning option. You can create aggregates that use a different disk size by using the advanced allocation option.
+
For help choosing a disk size, refer to link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure[Size your system in Azure^].
* If the public access to your storage account is disabled within the VNet, you cannot enable data tiering in your Cloud Volumes ONTAP system. For information, refer to link:reference-networking-azure.html#security-group-rules[Security group rules].
* You can choose a specific volume tiering policy when you create or edit a volume.
* If you disable data tiering, you can enable it on subsequent aggregates.
+
...
+
link:link:reference-configs-azure.html#ha-pairs[Learn more about supported configurations in Azure^].
...
*link:task-planning-your-config-azure.html[Planning your Cloud Volumes ONTAP configuration in Azure^]
*link:task-deploy-cvo-azure-mktplc.html[Deploy Cloud Volumes ONTAP in Azure from the Azure Marketplace^]
