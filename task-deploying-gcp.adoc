...
. *Preconfigured Packages*: Select one of the packages to quickly deploy a Cloud Volumes ONTAP system, or click *Create my own configuration*. ##The preconfigured packages vary with the supported VM types on the system. If you select a C3 VM type, then the Console shows packages that are optimized for C3 VMs, for example, configurations with only Hyperdisk Balanced disk types.##
+
