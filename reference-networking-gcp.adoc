...
summary: Set up your Google Cloud networking so Cloud Volumes ONTAP systems can operate properly. You learn about networking requirements, private IP addresses, VPCs, subnets, packet mirroring, firewall rules, and HA configurations for CVO and also Console agent requirements.
---
...
The Console creates four Google Cloud internal load balancers (TCP/UDP) that manage incoming traffic to the Cloud Volumes ONTAP HA pair. No setup is required from your end. We've listed this as a requirement simply to inform you of the network traffic and to mitigate any security concerns.
...
A shared VPC enables you to configure and centrally manage virtual networks across multiple projects. You can set up shared VPC networks in the _host project_ and deploy the Console agent and Cloud Volumes ONTAP virtual machine instances in a _service project_. 

https://cloud.google.com/vpc/docs/shared-vpc[Google Cloud documentation: Shared VPC overview^].

https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html[Review the required shared VPC permissions covered in Console agent deployment^]
...
The Console agent also contacts several endpoints for day-to-day operations. For information about the endpoints, refer to https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking[View endpoints contacted from the Console agent^] and https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html[Prepare networking for using the Console^].
...
|
\https://cloudbuild.googleapis.com/v1 (for only private mode deployments)
...
|Cloud Volumes ONTAP cannot communicate with Google Cloud service to perform specific operations for the Console in Google Cloud. 
...
https://cloud.google.com/vpn/docs/concepts/overview[Google Cloud documentation: Cloud VPN overview^].
...
TIP: Looking for information about the Console agent? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html[View firewall rules for the Console agent^]
...
.3+| AutoSupport | HTTPS | 443 | Node management LIF | mysupport.netapp.com | AutoSupport (HTTPS is the default)
...
| Configuration backups | HTTP | 80 | Node management LIF | \http://<console-agent-IP-address>/occm/offboxconfig | Send configuration backups to the Console agent. link:https://docs.netapp.com/us-en/ontap/system-admin/node-cluster-config-backed-up-automatically-concept.html[ONTAP documentation^]
...
Meanwhile, the predefined firewall rules created for the instances in VPC-1, VPC-2, and VPC-3 enable ingress communication over _all_ protocols and ports. These rules enable communication between HA nodes.
...
=== Infrastructure Manager configuration in private mode or protected deployments
If you want to deploy Cloud Volumes ONTAP 9.16.1 or later in private mode or in a project protected by VPC Service Controls, you need to make a few configuration changes so that Cloud Volumes ONTAP can use Google Cloud Infrastructure Manager as the deployment service instead of Deployment Manager, which Google will eventually deprecate.

.Before you begin

* Ensure that your Cloud Volumes ONTAP system is 9.16.1 or later. If it isn't, upgrade your system. Refer to https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html[Upgrade Cloud Volumes ONTAP] for instructions.
...
* Ensure that the Cloud Build API is enabled. Refer to https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment[Enable the Cloud Build API here^]. 
* Verify that the Console agent's service account has all standard permissions. Additionally, ensure that the service account has the `cloudbuild.workerpools.get` and `cloudbuild.workerpools.list` permissions. Refer to https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions[Google Cloud permissions for the Console agent^].

.Steps
. Create a private worker pool with this configuration in the same region as the Cloud Volumes ONTAP deployments. For information about creating a private worker pool, refer to https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool[Google Cloud documentation: Create and manage private pools^] and https://cloud.google.com/build/pricing[Google Cloud Build pricing^].
+
If you deploy Cloud Volumes ONTAP in a project protected by VPC Service Controls, create the private worker pool in the same project as the Console agent. The worker pool must be in a running state. Google Cloud's default public Cloud Build worker pool can't run inside a service perimeter.
+
The worker pool must have the following configuration:
+
* Machine type: e2-medium
* Disk size: 100 GB
* Assign external IP: False
* Network: Default or a private. 
* The subnet configured to access https://cloud.google.com/apis[Google Cloud APIs^]. Perform these steps to ensure that the subnet can access Google APIs:
.. Ensure "Private Google Access" is turned on for the subnet.
.. Go to *VPC Network level > Private Service Access Tab > Allocated IP ranges for services*.
..  Select *Allocate IP range* and allocate the internal IP range for the private connection to the Google Compute Service.
.. On *Private connection to services*, select *Create Connection*.
.. Select *Connected service producer = Google Cloud Platform*.
.. Assign the allocation for the private connection IP range you created in the previous step.
. Deploy this worker pool and keep it running for Cloud Volumes ONTAP management. Google Cloud uses this worker pool to run all Terraform operations in an isolated environment.
. When deploying Cloud Volumes ONTAP in private mode, select the name of this worker pool in the *GCP Worker Pool* field. Refer to link:task-deploying-gcp.html[Launch Cloud Volumes ONTAP in Google Cloud] for instructions.
+
If you deploy Cloud Volumes ONTAP in a project protected by VPC Service Controls, create the Cloud Volumes ONTAP system by using the API and include the `workerPool` parameter in the create request body.
+
[NOTE]
Including a worker pool isn't currently supported when you deploy Cloud Volumes ONTAP using the Console UI. For projects protected by VPC Service Controls, the API is the only supported way to deploy Cloud Volumes ONTAP.
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html[View networking requirements for the Console agent^]
...
For information about configuring proxy servers for the Console agent, refer to the https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html[Configure a Console agent to use a proxy server^]. 
...
. In the Google Cloud Console, locate your Cloud Volumes ONTAP system.
. Go to *Details > Networking > Network tags*.
. Add the tag used for the Console agent and save the configuration.
