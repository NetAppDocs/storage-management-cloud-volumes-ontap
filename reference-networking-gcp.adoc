...
.Steps
. Create a private worker pool with this configuration in the same region as the Cloud Volumes ONTAP deployments:
...
. When deploying Cloud Volumes ONTAP in private mode, select the name of this worker pool in the *GCP Worker Pool* field.
