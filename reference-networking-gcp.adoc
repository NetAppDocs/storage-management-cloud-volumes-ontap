...
=== Infrastructure Manager configuration in private-mode and VPC Service Controls-enabled deployments
...
+
If you deploy Cloud Volumes ONTAP in a project protected by VPC Service Controls, create the private worker pool in the same project as the Console agent. The worker pool must remain in the *RUNNING* state. Google Cloud's default public Cloud Build pool can't run inside a service perimeter.
+
...
+
[NOTE]
Passing a worker pool isn't currently supported in the Console UI. For VPC SC-protected projects, the API is the only supported way to deploy Cloud Volumes ONTAP.
