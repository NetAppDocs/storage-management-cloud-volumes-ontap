...
* `gs://netapp-cvo-infrastructure-manager-<project id>`: for storing  for ONTAP versions and SVM Terraform Templated used for new CVO deployments. Inside this, the `dm-to-im-convert` bucket contains the Cloud Volumes ONTAP Terraform files.
...
* Ensure that the Google Cloud APIs are enabled. Refer to https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis[Enable Google Cloud APIs^]. Ensure that along with the other APIs, you enable the Google Cloud Quotas API.
* Verify that the NetApp Console agent's service account has all required permissions. Refer to https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions[Google Cloud permissions for the Console agent^].
+
...
* If you are running the tool in a private mode, ensure that along with the other APIs, you have enabled the Cloud Build API. https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis[Enable Google Cloud APIs^].
* Ensure that you have completed the network configurations and created the worker pool for private mode deployments. Refer to link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments[Infrastructure Manager configuration for private mode deployments].
====
...
| TCP
| EGRESS
| Build operations only required for private mode
...
* OS: ##Based on your requirement, select either:##
...
. Download the conversion tool from the https://mysupport.netapp.com/site/tools/tool-eula/gcp-cvo-dm-to-im-convert-tool/download[NetApp Support site^] by using your NSS credentials: `wget <download link from NetApp Support site>`
. Extract the downloaded TAR file: `unzip <downloaded file name>`
...
[tab="SUSE Linux Enterprise Server"]

.SUSE Linux Enterprise Server
. Set up Python: `sudo update-alternatives --install /usr/bin/python3 python3 /usr/bin/python3.11 2`
. Install pip3 for installing the package: `python3.11 -m ensurepip --upgrade`
. Install Terraform:
+
[source,cli]
----
...
[%collapsible]
====
Specify the `--worker-pool` parameter to run the tool in private mode deployments. For worker pool configuration, refer to link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments[Infrastructure Manager configuration for private mode deployments].
[source,cli]
----
dmconvert \
--project-id=<the Google Cloud project ID for the Cloud Volumes ONTAP deployment> \
...
.After you finish
The tool displays a list of all Cloud Volumes ONTAP systems and SVM details. When it finishes running, you can see the statuses of all the converted systems. Each converted system appears in the Google Console under Infrastructure Manager in a _<system-name-imdeploy>_ format, indicating that the Console now uses Infrastructure Manager APIs to manage that Cloud Volumes ONTAP system.
[CAUTION]
Post conversion, do not delete the deployment object for Deployment Manager in the Google Cloud Console. This deployment object contains information of the Cloud Volumes ONTAP system that can be used to roll back the converted system.
