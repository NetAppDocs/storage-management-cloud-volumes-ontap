...
. [[create-vm]]Create a VM with following configurations. You run the tool on this VM.
+
...
----
+
. Install the conversion tool: `./install.sh`. This installs the tool in an isolated environment, `dmconvert-venv`, and validates if all the required software packages are installed.
