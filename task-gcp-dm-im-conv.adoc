...
* Ensure that you have completed the network configurations and created the worker pool for private mode deployments. Refer to link:reference-networking-gcp.html#infrastructure-manager-configuration-in-private-mode-or-protected-environments[Infrastructure Manager configuration for private mode deployments].
====
...
[%collapsible]
====
Specify the `--worker-pool` parameter to run the tool in private mode deployments. For worker pool configuration, refer to link:reference-networking-gcp.html#infrastructure-manager-configuration-in-private-mode-or-protected-environments[Infrastructure Manager configuration for private mode deployments].
[source,cli]
----
dmconvert \
--project-id=<the Google Cloud project ID for the Cloud Volumes ONTAP deployment> \
...
//BLUEXPDOC-1324: only storage.buckets.get was added. compute.instances.getSerialPortOutput was skipped because the filer didn't give log access that's needed for confirmation.
