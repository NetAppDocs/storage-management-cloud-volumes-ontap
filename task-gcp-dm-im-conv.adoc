...
`gcloud iam roles crea
te dmtoim-convert-tool-role --project=PROJECT_ID \
    --file=YAML_FILE_PATH`
...
----
+
. Install the conversion tool: `./install.sh`. This installs the tool in an isolated environment, `dmconvert-venv`, and verifies that all required software packages are installed.
