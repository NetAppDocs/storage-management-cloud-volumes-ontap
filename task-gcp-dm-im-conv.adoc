...
+
[[VM that you created for running the tool]]
...
If you don't want to proceed with the conversion, you can roll back to Cloud Deployment Manager by following these steps:

.Steps
. On the same <<VM that you created for running the tool>>, run this command:
+
[source,cli]
----
dmconvert \
...
. After the rollback is complete, verify that your Cloud Volumes ONTAP system is using