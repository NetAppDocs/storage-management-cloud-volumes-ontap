...
[.lead]
Google is about to deprecate Google Cloud Deployment Manager in favor of Cloud Infrastructure Manager. Therefore, you need to manually run a transition tool to convert your existing Cloud Volumes ONTAP deployments in Google Cloud from Deployment Manager to Infrastructure Manager. This is a one-time process, after which your systems will automatically start using Infrastructure Manager.

Beginning on January 12, 2026, new Cloud Volumes ONTAP deployments in Google Cloud can use Infrastructure Manager.
...
----
+
. Install the conversion tool: `./install.sh`. This validates the required software packages and installs the tool in an isolated python environment `dmconvert-venv`.
