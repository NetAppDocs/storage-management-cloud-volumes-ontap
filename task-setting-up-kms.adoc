...
+
. Enable access for the Console agent to the customer-manager encryption key (CMEK). Use one of the following methods:
...
+
. If the CMEK key is stored in Account B, the agent role for Account A needs explicit access to use that key. The KMS key policy in Account B grants that cross-account access.
...
+
[NOTE]
The key policy in Account B controls access to the key, and the IAM policy in Account A allows the Console agent role to make KMS API calls. If the configuration is missing for either of the accounts, access is restricted.
+
