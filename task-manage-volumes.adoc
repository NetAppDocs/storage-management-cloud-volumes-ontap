...
.Steps
. Select the System Manager view to resize a volume through ONTAP System Manager. Refer to link:task-administer-advanced-view.html#get-started-with-system-manager[Get started with System Manager].
...
TIP: The Console displays these Action Required messages when an aggregate has reached 90% used capacity. If data tiering is enabled, the messages display when an aggregate has reached 80% used capacity. By default, 10% free space is reserved for data tiering. link:task-tiering.html#change-the-free-space-ratio-for-data-tiering[Learn more about the free space ratio for data tiering^].
...
.. On the aggregate tile, click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon and then click *View aggregate details*.
...
+
For information, refer to link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html[Replicating data between systems^].
...
+
For information, refer to link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships[Managing data replication schedules and relationships^].
...
.. On the required aggregate tile, click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon and then *View aggregate details* to view the available capacity (provisioned size minus used aggregate capacity).
+
...
.. Select the aggregate, then click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon > *Add Disks*.
