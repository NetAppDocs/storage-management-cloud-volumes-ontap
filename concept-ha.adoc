...
Access to the instance:: When you create a Cloud Volumes ONTAP HA pair from the Console, you're prompted to provide a key pair for the mediator instance. You can use that key pair for SSH access using the `admin` user.
...
For more information, refer to link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs[AWS networking requirements for Cloud Volumes ONTAP HA in multiple AZs^].
...
NOTE: AWS Local Zones are supported when using the Console in standard and private modes. At this time, AWS Local Zones are not supported in restricted mode.
...
link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2[AWS Documentation: EC2 instance types in Local Zones^].
...
NOTE: You can set up an active-active configuration only when using the Console in the Storage System View.
...
You can easily identify the correct IP address through the _Mount Command_ option under the manage volumes panel in.
