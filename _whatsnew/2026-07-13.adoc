...
NetApp introduces identity and access management (IAM) roles for Cloud Volumes ONTAP, enabling role-based access control (RBAC) for better security and easier compliance. You can continue to log in to the NetApp Console using your existing credentials. However, authentication is now routed through identity federation, enabling the Console administrator to have more granular control over Cloud Volumes ONTAP operations. Every user must have one of these roles for performing any operation on Cloud Volumes ONTAP. Without a role, users can't view or access Cloud Volumes ONTAP instances. The following roles are now available:
...
[NOTE]
Beginning July 13, 2026, Cloud Volumes ONTAP roles can be assigned to existing users when a Console agent is first deployed or first upgraded in each organization. The seeding process for a new Cloud Volume ONTAP role is around 30 minutes, and once the seeding process completes, users with specific roles can start Cloud Volumes ONTAP workflows.
