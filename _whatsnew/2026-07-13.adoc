...
NetApp introduces identity and access management (IAM) roles for Cloud Volumes ONTAP, enabling role-based access control (RBAC) for better security and easier compliance. You can continue to log in to the NetApp Console using your existing credentials. However, authentication is now routed through identity federation, enabling the Console administrator to have more granular control over Cloud Volumes ONTAP user operations. Every user must have one of these roles for performing any operation on Cloud Volumes ONTAP. Without a role, users can't view or access Cloud Volumes ONTAP instances. The following roles are now available:
...
[NOTE]
A new Cloud Volume ONTAP role can take around an hour to be propagated within an organization after it is assigned to a user, and users can start Cloud Volumes ONTAP workflows only after the propagation completes. These roles are currently available in preview mode. In a future release, when the Console administrator is granted permissions for Cloud Volumes ONTAP user management and role assignment, NetApp activates these roles.
