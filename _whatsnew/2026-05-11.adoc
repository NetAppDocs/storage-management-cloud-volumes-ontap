...
If you have only a standalone AWS subscription associated with your AWS account in the NetApp Console and no NIS subscriptions, you can select only the Essentials license package during deployment. The data services *Backup and Recovery* option is unavailable for you.

.Both standalone and NIS subscriptions (existing customers)
If you have both NIS and standalone subscriptions associated with your AWS account, and you haven't set a billing preference, Cloud Volumes ONTAP usage continues to bill through NIS by default. This is also the default for any Cloud Volumes ONTAP instance running the Professional package, because the standalone listing does not support Professional. When both the subscriptions are associated, you can set a billing preference and choose how to bill each service.
