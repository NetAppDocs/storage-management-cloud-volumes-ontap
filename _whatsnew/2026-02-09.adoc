...
[cols=6*,options="header",width=100%]
|===
...
a| https://review.docs.netapp.com/us-en/storage-management-cloud-volumes-ontap_dm-im-tool/task-getting-started-gcp.html[Getting started with Cloud Volumes ONTAP in Google Cloud^]

|New agent and new deployment
| Create a new agent with and deploy a new Cloud Volumes ONTAP system.
|
|
| 
...
*  https://review.docs.netapp.com/us-en/storage-management-cloud-volumes-ontap_dm-im-tool/task-getting-started-gcp.html[Getting started with Cloud Volumes ONTAP in Google Cloud^]

|Existing agent and existing deployment in private mode  
| Upgrade to the latest agent by downloading the product installer from the NetApp Support Site and then manually installing the agent on your host, so that the agent can use the Infrastructure Manager APIs. Then convert the existing Cloud Volumes ONTAP systems to use Infrastructure Manager.
a| * Cloud Infrastructure Manager API 
* Cloud Quotas API
* Cloud Build API
