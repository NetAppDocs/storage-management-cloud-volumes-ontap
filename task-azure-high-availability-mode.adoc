...
[.lead]
You should enable Microsoft Azure's high-availability (HA) mode to reduce unplanned failover times and to enable NFSv4 support for Cloud Volumes ONTAP. If you enable this mode, your Cloud Volumes ONTAP HA nodes can achieve a low (60 seconds) recovery time objective (RTO) during unplanned failovers on CIFS and NFSv4 clients.
