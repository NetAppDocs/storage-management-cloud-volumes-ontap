...
ifdef::aws[]
* For AWS, you'll need a VPC Endpoint to Amazon Simple Storage Service (Amazon S3). <<Requirements to tier cold data to AWS S3,Learn more>>.
endif::aws[]
...
The custom role includes the permissions by default. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html[View Azure permission for the Console agent^]
...
//* You have entered the credentials for both the tenants in the Console for cross-validation. Refer to https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html[Manage Azure credentials and marketplace subscriptions for the Console^].
...
+
For help with replicating data, refer to https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html[Replicating data to and from the cloud^].
...
. Click the image:icon-action.png["An icon that shows three horizontal dots for additional actions"] icon for the Console agent that manages your Cloud Volumes ONTAP system.
